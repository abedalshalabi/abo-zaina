...
const AdminCategories = () => {
  ...
  const handleBulkDelete = async () => {
    ...
    if (window.confirm(`هل أنت متأكد من حذف ${selectedCategories.length} فئة؟`)) {
      try {
        await Promise.all(
          selectedCategories.map(id => adminCategoriesAPI.deleteCategory(id.toString()))
        );
              ...
              <table className="w-full">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {organizeCategoriesHierarchically(categories).map((category) => (
                    <tr key={category.id} className={`${category.level === 0 ? 'bg-gray-100 border-l-4 border-l-green-400 hover:bg-gray-200' : 'hover:bg-blue-50'} transition-colors duration-200`}>
                      {/* Checkbox */}
